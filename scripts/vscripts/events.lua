...
function CAghanim:InitBlessings( hHero )
	...
	if tBlessings == nil then
		print("original blessing is nil")

		tBlessings = _G.BLESSING_TREE_UPGRADES
	end
	if not tBlessings then
        print("no original and custom blessings!")
        return
    end
	local players = {}
	...
	for szBlessingType,nBlessingValue in pairs( tBlessings ) do
		...
		local adjustedBlessingValue = nBlessingValue
		local isStartingBlessingValue = _G.BLESSING_TREE_BONUS_PER_MISSING_PLAYER and _G.BLESSING_TREE_BONUS_PER_MISSING_PLAYER[szBlessingType]

        if isStartingBlessingValue then
            adjustedBlessingValue = adjustedBlessingValue + _G.BLESSING_TREE_BONUS_PER_MISSING_PLAYER[szBlessingType] * (_G.AGHANIM_PLAYERS - #players)
        end
		if tBlessingDefinition ~= nil then
			if tBlessingDefinition.grant_blessing ~= nil then
				print( "Granting blessing " .. szBlessingType .. ":" .. tostring( adjustedBlessingValue ) )
				tBlessingDefinition.grant_blessing( nPlayerID, hHero, adjustedBlessingValue )
			elseif tBlessingDefinition.modifier_name ~= nil then
				print( "Granting blessing " .. szBlessingType .. ":" .. tostring( adjustedBlessingValue ) .. " as modifier " .. tBlessingDefinition.modifier_name )
				local hModifier = hHero:AddNewModifier( hHero, nil, tBlessingDefinition.modifier_name, {} )
				
				hModifier:SetStackCount( adjustedBlessingValue )
			else
...
function CAghanim:OnEntityKilled_EnemyCreature( event )
	local hDeadCreature = EntIndexToHScript( event.entindex_killed )
    local hAttackerUnit = EntIndexToHScript( event.entindex_attacker )
	local dropChance = 15
	if RollPercentage(dropChance) then
        local newItem = CreateItem( "item_madstone_bundle", nil, nil )
        newItem:SetPurchaseTime( 0 )
    
        local vPos = hDeadCreature:GetAbsOrigin()
        local drop = CreateItemOnPositionSync( vPos, newItem )

        if drop then
            drop:SetModelScale(1.5)
        end
    
        local dropTarget = FindPathablePositionNearby(vPos, 150, 350)
        newItem:LaunchLoot( false, 350, 0.75, dropTarget, nil)
    end
end
---------------------------------------------------------
...
function CAghanim:OnEntityKilled_PlayerHero( event )
	...
	if killedHero:GetUnitName() == "npc_dota_hero_nevermore" then
		local death_mod = killedHero:FindModifierByName("modifier_aghsfort_nevermore_necromastery_collection")
	...
	if bImmortal then
        SetImmortalHero(killedHero, false)
    else
        print(killedHero.nRespawnsRemaining)
		...
		if i ~= nil then
			...
			if hTriggerEntity.bHasBeenTriggered == nil then
				...
				if room ~= nil then
					...
					for nPlayerID = 0, AGHANIM_PLAYERS - 1 do 
						local hPlayerHero = PlayerResource:GetSelectedHeroEntity( nPlayerID )
						if hPlayerHero and hPlayerHero:GetName() == "npc_dota_hero_alchemist" then
							if hPlayerHero:HasModifier("modifier_alchemist_the_buffett") then
								local stackcount = hPlayerHero:GetModifierStackCount("modifier_alchemist_the_buffett", hPlayerHero)	
								--print(stackcount)
								local pct = math.random(0,20)
								local goldreturn = stackcount*10*pct
								--print("return gold")
								--print(goldreturn)
								hPlayerHero:ModifyGoldFiltered(goldreturn, true, DOTA_ModifyGold_Unspecified)
								EmitGlobalSound("General.Sell")
								local gameEvent = {}
								gameEvent["message"] = "#DOTA_Hud_EventAlchemist1"
								gameEvent["player_id"] = hPlayerHero:GetPlayerOwnerID()
								gameEvent["teamnumber"] = -1
								gameEvent[ "int_value" ] = stackcount*100
								FireGameEvent( "dota_combat_event_message", gameEvent )
								local gameEvent1 = {}
								gameEvent1["message"] = "#DOTA_Hud_EventAlchemist"
								gameEvent1["player_id"] = hPlayerHero:GetPlayerOwnerID()
								gameEvent1["teamnumber"] = -1
								gameEvent1[ "int_value" ] = goldreturn
								FireGameEvent( "dota_combat_event_message", gameEvent1 )
								hPlayerHero:RemoveModifierByName("modifier_alchemist_the_buffett")
							end
...
function CAghanim:OnPlayerChat( event )
	...
	elseif sChatMsg:find( '^-hell$' ) then
		if self._arrHard == nil then
			self._arrHard = {}
		end
		self._arrHard[nPlayerID] = true
		local count = 0
		for id, v in pairs(self._arrHard) do
			count = count + 1           
		end 
		if count >= #(self:GetConnectedPlayers()) then              
			...
			CustomNetTables:SetTableValue( "hell_mode", "hell", kv )
			local hell = CustomNetTables:GetTableValue( "hell_mode", "hell" )
			print(hell.hell)
			GameRules:SendCustomMessageToTeam("Ascension level has been set to HELLMODE", DOTA_TEAM_GOODGUYS, 0, DOTA_TEAM_GOODGUYS)
			-- GameRules:SendCustomMessage("now you are god!", DOTA_TEAM_GOODGUYS, 0)   
		else
			GameRules:SendCustomMessageToTeam("Needs ALL players to type -hell to enter HELLMODE", DOTA_TEAM_GOODGUYS, 0, DOTA_TEAM_GOODGUYS)
		end
	end
end
