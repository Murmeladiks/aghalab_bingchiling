...
function CPortalSpawnerV2:SpawnSingleUnitType( rgUnitInfo, vLocation )
	...
	for i=1,rgUnitInfo.Count do
		...
		if hell and  hell.hell == 0 then
			...
			if  rgUnitInfo.EntityName =="npc_dota_pinecone_warrior" then
				local unitname = "npc_dota_pinecone_warrior1" 
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_bear_cave_ursa_cub" then
				local unitname = "npc_dota_creature_bear_cave_ursa_cub1" 
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_magma_minion" then
				local unitname = "npc_dota_creature_magma_minion1" 
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_diregull_small" then
				local unitname = "npc_dota_creature_diregull_small1"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_large_eimermole" then
				local nLevel = GameRules.Aghanim:GetAscensionLevel()
				local bristleback_quill_spray = hUnit:AddAbility("bristleback_quill_spray")
				local bristleback_bristleback = hUnit:AddAbility("bristleback_bristleback")
				local bristleback_warpath = hUnit:AddAbility("bristleback_warpath")
				bristleback_quill_spray:SetLevel(nLevel)
				bristleback_bristleback:SetLevel(nLevel)
				bristleback_warpath:SetLevel(nLevel-1)
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_phantom_lancer" then
				local item  = CreateItem("item_vladmir", hUnit, hUnit)
				hUnit:AddItem(item)
				local item1 = CreateItem("item_diffusal_blade", hUnit, hUnit)
				hUnit:AddItem(item1)
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_dungeon_bat" then
				local unitname = "npc_dota_creature_dungeon_bat1"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_morphling_big" then
				local nLevel = GameRules.Aghanim:GetAscensionLevel()
				local spectre_desolate = hUnit:AddAbility("spectre_desolate")
				spectre_desolate:SetLevel(nLevel)
				
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_life_stealer" then
				local nLevel = GameRules.Aghanim:GetAscensionLevel()
				local life_stealer_ghoul_frenzy = hUnit:AddAbility("life_stealer_ghoul_frenzy")
				life_stealer_ghoul_frenzy:SetLevel(nLevel)
				
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_aziyog_underlord" then
				local nLevel = GameRules.Aghanim:GetAscensionLevel()
				local abyssal_underlord_atrophy_aura = hUnit:AddAbility("abyssal_underlord_atrophy_aura")
				abyssal_underlord_atrophy_aura:SetLevel(nLevel)	
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_large_ogre_seal" then
				local unitname = "npc_dota_creature_ogre_magi1"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
				if hellunit then
					local nLevel = GameRules.Aghanim:GetAscensionLevel()
					hellunit:CreatureLevelUp( nLevel ) 
					FindClearSpaceForUnit(hellunit, vSpawnPos + RandomVector(250), true)
				end
				local hellunit1 = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
				if hellunit1 then 
					local nLevel = GameRules.Aghanim:GetAscensionLevel()
					hellunit1:CreatureLevelUp( nLevel )
					FindClearSpaceForUnit(hellunit1, vSpawnPos + RandomVector(250), true)
				end
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_venomancer" then
				local nLevel = GameRules.Aghanim:GetAscensionLevel()
				local viper_corrosive_skin = hUnit:AddAbility("viper_corrosive_skin")
				viper_corrosive_skin:SetLevel(nLevel)	
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_shadow_shaman" then
				local unitname = "npc_dota_creature_puck"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
				if hellunit then 
					local nLevel = GameRules.Aghanim:GetAscensionLevel()
					hellunit:CreatureLevelUp( nLevel )
			...
			if  rgUnitInfo.EntityName =="npc_dota_creature_shroom_giant" then
				local nLevel = GameRules.Aghanim:GetAscensionLevel()
				local elder_titan_natural_order = hUnit:AddAbility("elder_titan_natural_order")
				elder_titan_natural_order:SetLevel(nLevel)	
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_omniknight_egg_smasher" then
				local unitname = "npc_dota_creature_dragon_knight"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
				if hellunit then 
					local nLevel = GameRules.Aghanim:GetAscensionLevel()
					hellunit:CreatureLevelUp( nLevel )
			...
			if  rgUnitInfo.EntityName =="npc_dota_creature_large_ogre_seal_diretide" then
				local unitname = "npc_dota_creature_warlock"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_outworld" then
				local unitname = "npc_dota_creature_lich"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_polarity_ghost_captain_positive" then
				local unitname = "npc_dota_creature_polarity_ghost_captain_negative"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_polarity_ghost_captain_negative" then
				local unitname = "npc_dota_creature_polarity_ghost_captain_positive"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_rock_golem_a" then
				local item  = CreateItem("item_stony_coat", hUnit, hUnit)
				hUnit:AddItem(item)
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_batrider" then
				local unitname = "npc_dota_spider_sac"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )
			end
			if  rgUnitInfo.EntityName =="npc_dota_creature_snapfire" then
				local unitname = "npc_dota_creature_kidnap_spider"
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )	
			end
			...
			if  rgUnitInfo.EntityName =="npc_aghsfort_creature_dire_assault_captain" then
				...
				local hellunit = CreateUnitByName( unitname, vSpawnPos, true, nil, nil, rgUnitInfo.Team )	
				local item  = CreateItem("item_moon_shard", hUnit, hUnit)
				hellunit:AddItem(item)
			end
