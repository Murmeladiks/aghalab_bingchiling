...
function modifier_bristleback_quillspray_thinker_pf:OnCreated(kv)
	...
	self.quill_stack_damage =
		...
		self:GetAbility():GetSpecialValueFor("special_bonus_unique_bristleback_2")
	...
	self.attempt_bleeding = kv.bPassive == 1 and self.caster:HasAbility("pathfinder_bristleback_bristleback_magical_bleed")
...
function modifier_bristleback_quillspray_thinker_pf:OnIntervalThink()
	...
	local bleed_chance = self:GetAbility():GetSpecialValueFor("pathfinder_bristleback_bristleback_magical_bleed", "chance")
	local bleed_duration = self:GetAbility():GetSpecialValueFor("pathfinder_bristleback_bristleback_magical_bleed", "duration")
	local bDirectionalQuills =  self.caster:HasAbility("pathfinder_bristleback_quill_spray_directional_quills")
	local nAttackChance = self:GetAbility():GetSpecialValueFor("pathfinder_bristleback_quill_spray_directional_quills", "attack_projectile_chance")
