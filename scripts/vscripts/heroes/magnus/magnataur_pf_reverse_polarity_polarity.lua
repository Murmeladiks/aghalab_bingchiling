...
function magnataur_pf_reverse_polarity_polarity:OnAbilityUpgrade(hUpgradeAbility)
	-- if self:GetCaster():GetHeroFacetID() == 2 then return end
...
function magnataur_pf_reverse_polarity_polarity:OnSpellStart()
	...
	if hCaster:FindAbilityByName("aghsfort_special_magnataur_reverse_polarity_steroid")  ~= nil then
		hCaster:AddNewModifier(hCaster, self, "modifier_magnataur_pf_reverse_polarity_steroid", {duration = 15, nMult = 1})
	end
