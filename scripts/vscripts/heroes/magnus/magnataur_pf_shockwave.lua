...
function magnataur_pf_shockwave:OnSpellStart()
	...
	if  not hCaster:HasAbility("aghsfort_special_magnataur_shockwave_multishot")  then 
		print("special ability not added")
		return
	end
...
function magnataur_pf_shockwave:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
	...
	if hCaster:HasAbility("aghsfort_special_magnataur_shockwave_damage_reduction") then
		local nDur = self:GetSpecialValueFor("basic_slow_duration") + 2.5
...
function magnataur_pf_shockwave:Boomerang(vLocation, ExtraData)
	...
	if not hCaster:HasAbility("aghsfort_special_magnataur_shockwave_boomerang") or ExtraData.bReturning == 1 then return end
