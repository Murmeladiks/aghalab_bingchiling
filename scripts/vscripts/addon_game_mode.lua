...
function LinkModifiers()
    ...
    LinkLuaModifier( "modifier_spectre_haunt_single_fix", "heroes/spectre/modifier_spectre_haunt_single_fix", LUA_MODIFIER_MOTION_BOTH )
    LinkLuaModifier( "modifier_templar_assassin_psionic_trap_fix", "heroes/templar_assassin/modifier_templar_assassin_psionic_trap_fix", LUA_MODIFIER_MOTION_BOTH )
    LinkLuaModifier( "modifier_undying_decay_fix", "heroes/undying/modifier_undying_decay_fix", LUA_MODIFIER_MOTION_BOTH )
    LinkLuaModifier( "modifier_omniknight_facet_fix", "heroes/omniknight/modifier_omniknight_facet_fix", LUA_MODIFIER_MOTION_BOTH )
end
...
function CAghanim:InitGameMode()
    ...
    --there is no tournament any more
    ...
    self.bWonGame = false
    -- assume there is no new player
    self.bHasAnyNewPlayers = false
    -- so we dont have to set new players
    ...
    ListenToGameEvent( "dota_player_pick_hero", Dynamic_Wrap( CAghanim, "OnPlayerPickHero" ), self )
    ...
    for nPlayerID = 0, AGHANIM_PLAYERS-1 do
        ...
        --初始化数据表
        --self:_InitPlayerStatsTable(nPlayerID)
    end
    ...
    --always set ascension level as 4
	if self.bIsInTournamentMode == true then
    ...
    --dummy unit spawn vector
    self.startPlaceVector = Vector( -8195, -13255, 640)
    --create dummy unit to collect statistics
    local statsUnit = CreateUnitByName("npc_dota_statistics_dummy", self.startPlaceVector, false, nil, nil, DOTA_TEAM_GOODGUYS)
    if statsUnit then
        print("add stats unit")
    end

    self.BristlebackItems = {}
    self.hAutoChannelEventOutpostBuff = nil 
    SendToServerConsole("dota_max_physical_items_purchase_limit 9999")
    SendToServerConsole("dota_max_physical_items_drop_limit 9999")
    SendToServerConsole("dota_pause_force_unpause_time 3600")
    SendToServerConsole("dota_pause_count 999")
end

--------------------------------------------------------------------------------
function CAghanim:OnPlayerPickHero( event )
    local hPlayerHero = EntIndexToHScript( event.heroindex )
    if hPlayerHero ~= nil then
        if hPlayerHero:GetName() == "npc_dota_hero_alchemist" then
            local alchemist_the_buffett = hPlayerHero:FindAbilityByName("alchemist_the_buffett")
            alchemist_the_buffett:SetLevel(1)
            alchemist_the_buffett:SetHidden(true)
        end
        if hPlayerHero:GetName() == "npc_dota_hero_spectre" then
            hPlayerHero:AddNewModifier(hPlayerHero,nil,"modifier_spectre_haunt_single_fix",{})
        end
        if hPlayerHero:GetName() == "npc_dota_hero_templar_assassin" then
            hPlayerHero:AddNewModifier(hPlayerHero,nil,"modifier_templar_assassin_psionic_trap_fix",{})
        end
        if hPlayerHero:GetName() == "npc_dota_hero_undying" then
            hPlayerHero:AddNewModifier(hPlayerHero,nil,"modifier_undying_decay_fix",{})
            local aghsfort_special_undying_consume_zombies_dummy = hPlayerHero:FindAbilityByName("aghsfort_special_undying_consume_zombies_dummy")
            aghsfort_special_undying_consume_zombies_dummy:SetLevel(1)
            aghsfort_special_undying_consume_zombies_dummy:SetHidden(true)
        end
        if hPlayerHero:GetName() == "npc_dota_hero_omniknight" then
            hPlayerHero:AddNewModifier(hPlayerHero,nil,"modifier_omniknight_facet_fix",{})
        end
    end
end
...
function CAghanim:ApplyGlobalAscensionModifiers()
    if self.nAscensionLevel >= AGHANIM_ASCENSION_MAGICIAN then 
        ...
        for nPlayerID = 0, AGHANIM_PLAYERS - 1 do
            ...
            if hPlayerHero ~= nil then
                hPlayerHero.nRespawnsRemaining = self.nStartingLives + 1
...
-- we dont set hero availability and compute always return
...
function CAghanim:GrantPlayerPoints( nPlayerID, nDesiredPoints, bBattlePoints, szReason )
    ...
    if not bBattlePoints then
        local nActionGranted = 1 --PlayerResource:GetNetworkedEventActionClaimCountByName( nPlayerID, 39, "labyrinth_arcane_fragment_bonus_rate" )
...
-- function CAghanim:_InitPlayerStatsTable(nPlayerID)
...
--     CustomNetTables:SetTableValue( "players_stats", string.format( "%d", nPlayerID ), self._vPlayerStats[ nPlayerID ])
-- end