...
function CAghanim:DamageFilter( filterTable )
	...
	local hAttackerUnit = EntIndexToHScript( filterTable["entindex_attacker_const"] )
    local flDamage = filterTable["damage"]
    local damageType = filterTable["damagetype_const"]
    local victim = nil
	if filterTable["entindex_victim_const"] then
...
function CAghanim:ItemAddedToInventoryFilter( filterTable )
	...
	if hItem and hero then
        local itemName = hItem:GetAbilityName()

        if (hItem:IsNeutralDrop() or hItem.validNeutralItemDrop) then
            local canAssignNeutralItemToPlayer = true

            if hItem.isNeutralToken and hItem:GetPurchaser() and hItem:GetPurchaser():GetPlayerOwnerID() ~= hero:GetPlayerOwnerID() then
                canAssignNeutralItemToPlayer = false
            end

            if canAssignNeutralItemToPlayer then
                self:AssignNeutralItemToPlayer(hero, hItem)
            end
        end
