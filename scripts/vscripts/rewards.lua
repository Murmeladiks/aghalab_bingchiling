...
LinkLuaModifier( "modifier_count_upgrade_reroll", "modifiers/modifier_count_upgrade_reroll", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_nogold_upgrade_reroll", "modifiers/modifier_nogold_upgrade_reroll", LUA_MODIFIER_MOTION_NONE )
...
function OnRewardReroll( eventSourceIndex, data )
	...
	if hPlayerHero then
		...
		if ( hBuff and hBuff:GetStackCount() > 0 ) then
			...
			FireGameEvent( "dota_combat_event_message", gameEvent )
		elseif  hBuff and hBuff:GetStackCount() == 0 then
			hPlayerHero:AddNewModifier(nil,nil,"modifier_count_upgrade_reroll",{})
			local countbuff  = hPlayerHero:FindModifierByName( "modifier_count_upgrade_reroll" )
			
				
			local times = countbuff:GetStackCount()
			local count = 1
			for i = 1, times do
				count = count + i
			end
			local consumePrice =  count*10
			
			
		
			
			if hPlayerHero:GetGold() < consumePrice  then
				--hPlayerHero:EmitSound("General.NoGold")
				if not hPlayerHero:HasModifier("modifier_nogold_upgrade_reroll") then
					hPlayerHero:AddNewModifier(nil,nil,"modifier_nogold_upgrade_reroll",{})
				end
				local gameEvent2 = {}
				gameEvent2["message"] = "#DOTA_Hud_EventNoGold"
				gameEvent2["player_id"] = hPlayerHero:GetPlayerOwnerID()
				gameEvent2["teamnumber"] = -1
				FireGameEvent( "dota_combat_event_message", gameEvent2 )
				
			elseif hPlayerHero:GetGold() >= consumePrice then
				
				local countbuff  = hPlayerHero:FindModifierByName( "modifier_count_upgrade_reroll" )
				countbuff:SetStackCount( countbuff:GetStackCount() + 1 )
				if hPlayerHero:HasModifier("modifier_nogold_upgrade_reroll") then 
					hPlayerHero:RemoveModifierByName("modifier_nogold_upgrade_reroll")
				end
				hPlayerHero:SpendGold(consumePrice, DOTA_ModifyGold_Unspecified)
				local gameEvent1 = {}
				gameEvent1["message"] = "#DOTA_Hud_EventRerollPay"
				gameEvent1["player_id"] = hPlayerHero:GetPlayerOwnerID()
				gameEvent1["teamnumber"] = -1
				gameEvent1[ "int_value" ] = consumePrice
