...
function lycan_boss_rupture_ball:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		if self:GetCaster() and not self:GetCaster():IsNull() and hTarget ~= nil and ( not hTarget:IsMagicImmune() ) 
			and ( not hTarget:IsInvulnerable() ) and hTarget:GetUnitName() ~= "npc_dota_forest_camp_chief" 
		then
