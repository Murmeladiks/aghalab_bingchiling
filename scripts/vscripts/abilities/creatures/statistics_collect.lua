...
statistics_collect = class({})

LinkLuaModifier( "modifier_statistics_collect", "abilities/creatures/statistics_collect", LUA_MODIFIER_MOTION_NONE )
...
function statistics_collect:GetIntrinsicModifierName()
	return "modifier_statistics_collect"
end

-----------------------------------------------------------------------------------------

modifier_statistics_collect = class({})

------------------------------------------------------------------------------

function modifier_statistics_collect:IsPurgable()
	return false
end

------------------------------------------------------------------------------

function modifier_statistics_collect:OnCreated( kv )
    if IsServer() then
        self.excludedUnits = {
			npc_dota_crate = true,
			npc_dota_vase = true,
...
function modifier_statistics_collect:CheckState()
	local state =
	{
...
function modifier_statistics_collect:DeclareFunctions()
	local funcs = 
	{
...
function modifier_statistics_collect:OnTakeDamage( params )
    if IsServer() then
...
-- function modifier_statistics_collect:OnDeath( params )
--     if IsServer() then
--         if params.attacker == self:GetParent() or params.unit == self:GetParent() then
--             return
--         end
