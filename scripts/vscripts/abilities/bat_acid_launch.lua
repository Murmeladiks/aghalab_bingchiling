...
function bat_acid_launch:IsHiddenAbilityCastable()
	return true
end
...
function bat_acid_launch:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		...
		self.thinker = CreateModifierThinker( self:GetCaster(), self, "modifier_alchemist_acid_spray_thinker", { duration = fDuration }, vLocation, self:GetCaster():GetTeamNumber(), false )
...
function bat_acid_launch:OnOwnerDied()
	if IsServer() then
		if self.thinker then
