...
function CMapEncounter_Hub:OnAscensionLevelSelected( event )
	...
	if (event.level - 1) < 3 then
		local connectedPlayers = CAghanim:GetConnectedPlayers()
			for _, nPlayerID in pairs(connectedPlayers) do
				if PlayerResource:IsValidPlayerID(nPlayerID) then
					local hero = PlayerResource:GetSelectedHeroEntity( nPlayerID )
					local item_cursed_item_slot = hero:FindItemInInventory("item_cursed_item_slot")
					hero:RemoveItem(item_cursed_item_slot)
					local item_cursed_item_slot = hero:FindItemInInventory("item_cursed_item_slot")
					hero:RemoveItem(item_cursed_item_slot)
					local item_cursed_item_slot = hero:FindItemInInventory("item_cursed_item_slot")
					hero:RemoveItem(item_cursed_item_slot)
...
function CMapEncounter_Hub:CustomStartupCompleted()
    ...
    if self.hRoom:GetDepth() < 3 then   
        ...
        for _, nPlayerID in pairs(connectedPlayers) do
			...
			-- 	if self:containsValue(Vips, steamid) then
...
function CMapEncounter_Hub:containsValue(table, value)
