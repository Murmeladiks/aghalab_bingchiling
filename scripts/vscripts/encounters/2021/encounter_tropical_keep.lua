...
function CMapEncounter_TropicalKeep:OnEncounterLoaded()
	...
	if #self.hAllyEnts == 0 then
		...
		if hell and  hell.hell == 0 then
			self.henemy = CreateUnitByName( "npc_dota_creature_friendly_radiant_guard1", hAllyEnt:GetOrigin(), false, nil, nil, DOTA_TEAM_BADGUYS )
			if self.henemy then 
				FindClearSpaceForUnit(self.henemy, hAllyEnt:GetOrigin() + RandomVector(250), true)
