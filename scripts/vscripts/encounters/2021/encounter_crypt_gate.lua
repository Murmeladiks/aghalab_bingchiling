...
function CMapEncounter_CryptGate:OpenGate()
	...
	if hRelays then
		for _, hRelay in pairs( hRelays ) do
			local pos = hRelay:GetAbsOrigin()
			local hell = CustomNetTables:GetTableValue( "hell_mode", "hell" )
			if hell and  hell.hell == 0 then
				self.henemy = CreateUnitByName( "npc_dota_undead_woods_skeleton_king", pos, false, nil, nil, DOTA_TEAM_BADGUYS )
				if self.henemy then 
					FindClearSpaceForUnit(self.henemy, pos + RandomVector(250), true)
					local nLevel = self:GetRoom():GetEliteRank() + GameRules.Aghanim:GetAscensionLevel()
					self.henemy:CreatureLevelUp( nLevel )
				end
				self.henemy1 = CreateUnitByName( "npc_dota_undead_woods_skeleton_king", pos, false, nil, nil, DOTA_TEAM_BADGUYS )
				if self.henemy1 then 
					FindClearSpaceForUnit(self.henemy1, pos + RandomVector(250), true)
					local nLevel = self:GetRoom():GetEliteRank() + GameRules.Aghanim:GetAscensionLevel()
					self.henemy1:CreatureLevelUp( nLevel )
				end
				self.henemy2 = CreateUnitByName( "npc_dota_undead_woods_skeleton_king", pos, false, nil, nil, DOTA_TEAM_BADGUYS )
				if self.henemy2 then 
					FindClearSpaceForUnit(self.henemy2, pos + RandomVector(250), true)
					local nLevel = self:GetRoom():GetEliteRank() + GameRules.Aghanim:GetAscensionLevel()
					self.henemy2:CreatureLevelUp( nLevel )
				end
				self.henemy3 = CreateUnitByName( "npc_dota_undead_woods_skeleton_king", pos, false, nil, nil, DOTA_TEAM_BADGUYS )
				if self.henemy3 then 
					FindClearSpaceForUnit(self.henemy3, pos + RandomVector(250), true)
					local nLevel = self:GetRoom():GetEliteRank() + GameRules.Aghanim:GetAscensionLevel()
					self.henemy3:CreatureLevelUp( nLevel )
				end
			end 
