...
function CMapEncounter_PrimalBeast:constructor( hRoom, szEncounterName )
	...
	self:AddSpawner( CDotaSpawner( self.szBossSpawner, self.szBossSpawner,
		{ 
			{
				EntityName = "npc_dota_boss_void_spirit",--"npc_dota_creature_aghsfort_primal_beast_boss",
...
--------------------------------------------------------------------------------
function CMapEncounter_PrimalBeast:GetCirclePoints(centerX, centerY, radius, count)
	local angleStep = (2 * math.pi) / count -- 计算两点之间的角度
    local points = {}
    for i = 1, count do
        local angle = (i - 1) * angleStep -- 计算当前点的角度
        local x = centerX + radius * math.cos(angle) -- 根据角度计算x坐标
        local y = centerY + radius * math.sin(angle) -- 根据角度计算y坐标
...
function CMapEncounter_PrimalBeast:Start()
	...
	local centerX= vBossPos.x
	local centerY = vBossPos.y
	local radius = 2000
	local hEarthSpiritSpawners = self:GetCirclePoints(centerX, centerY, radius, 8)
	
	self.hEarthSpirits = {} -- had some issue finding these units from Void's ability, so just track them as they get made
	for _, vSpawnPos in pairs( hEarthSpiritSpawners ) do
...
function CMapEncounter_PrimalBeast:GetPreviewUnit()
	return "npc_dota_boss_void_spirit"--"npc_dota_creature_aghsfort_primal_beast_boss"
end
