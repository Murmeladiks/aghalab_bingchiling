...
function modifier_brewmaster_split:OnDestroy()
	if IsServer() then	
		...
		if hell and  hell.hell == 0 then
			local voidspirit_name = "npc_dota_brewmaster_void_unit"
			local voidspirit_origin = vPos
			local voidspirit = CreateUnitByName(voidspirit_name, voidspirit_origin, true, self:GetParent(), nil, DOTA_TEAM_BADGUYS)
			local voidspiritfx_name = "particles/units/heroes/hero_brewmaster/brewmaster_void_ambient.vpcf"
			local voidspiritfx = ParticleManager:CreateParticle( voidspiritfx_name, PATTACH_ABSORIGIN_FOLLOW, voidspirit )
			ParticleManager:SetParticleControlEnt( voidspiritfx, 0, voidspirit, PATTACH_ABSORIGIN_FOLLOW, nil, voidspirit:GetOrigin(), true )
			voidspirit:SetInitialGoalEntity( target )
		end
		...
		FireGameEvent("entity_killed",
			...
			entindex_killed = self:GetParent():entindex(),
		
			entindex_attacker = self:GetParent():entindex(),
