LinkLuaModifier( "modifier_ascension_chilling_touch_debuff", "modifiers/modifier_ascension_chilling_touch_debuff", LUA_MODIFIER_MOTION_NONE )
	...
	if IsServer() then
		if self:GetParent() == params.attacker then
			...
			if hTarget ~= nil and hTarget:IsMagicImmune() == false then
				hTarget:AddNewModifier( self:GetCaster(), self:GetAbility(), "modifier_ascension_chilling_touch_debuff", { duration = self.duration } )
			end
		end
	end
