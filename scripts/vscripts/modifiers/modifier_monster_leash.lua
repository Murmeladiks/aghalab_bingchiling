...
function modifier_monster_leash:OnIntervalThink()
	...
	if ( vOrigin - vClampedPos ):Length2D() > 1.0 then
		...
		if self.nLeashCounter < 2 then
			if not (self:GetParent():GetUnitName() =="npc_dota_arrow_trap" or  self:GetParent():GetUnitName() =="npc_dota_breathe_fire_trap") then
				self.flKillStartTime = -1
	...
	if ( GameRules:GetGameTime() - self.flKillStartTime ) >= 3 and self:GetParent():GetUnitName() ~= "npc_dota_creature_aghsfort_primal_beast_boss" and self:GetParent():GetUnitName() ~= "npc_dota_creature_leshrac" and self:GetParent():GetUnitName() ~= "npc_dota_arrow_trap" and self:GetParent():GetUnitName() ~= "npc_dota_breathe_fire_trap" then
