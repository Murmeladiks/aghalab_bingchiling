	...
	purification_potion =
	{
		grant_blessing = function( nPlayerID, hHero, nBlessingValue )
			if nBlessingValue > 0 then
				...
				--table.insert(vPotionList, "item_tome_of_greater_knowledge")
		...
		grant_blessing = function( nPlayerID, hHero, nBlessingValue )
			...
			for i = 1, math.floor( nBlessingValue / 3 ) do
				if nAttribute == 0 then
					...
					GrantItemDropToHero( hHero, "item_tome_of_greater_knowledge" )
...
_G.BLESSING_TREE_UPGRADES = {
    ...
    stat_int = 4,
    stat_str = 4,
    purification_potion = 1,
    extra_life = 1,

    --bottle
    bottle_charges = 1,
    bottle_regen_duration = 2,
    bottle_regen_movement_speed = 30,

    --hero stats
    death_detonation = 150,
    ...
    cast_range = 40,
    stat_spell_amp = 8,
    -- debuff_duration_increase = 6, -- modifier is built-in and can't be hide
    respawn_invulnerability = 2,
    respawn_haste = 45,
    respawn_attack_speed = 40,
    low_hp_outgoing_damage = 8,
    respawn_time_reduction = 0.30,
...
_G.BLESSING_TREE_BONUS_PER_MISSING_PLAYER = {
    starting_gold = 1500,
    start_tome = 3,
    stat_agi = 2,
    stat_int = 2,
