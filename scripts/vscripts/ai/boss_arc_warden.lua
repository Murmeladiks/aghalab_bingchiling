...
function CBossArcWarden:SetEncounter( hEncounter )
	CBossBase.SetEncounter( self, hEncounter )
		local vPosition = Vector(8000, 6016, 768)
		local hGuardianBoss1 = CreateUnitByName( "npc_dota_creature_temple_guardian", vPosition, true, nil, nil, DOTA_TEAM_BADGUYS )
	 	local hGuardianBoss2 = CreateUnitByName( "npc_dota_creature_temple_guardian", vPosition, true, nil, nil, DOTA_TEAM_BADGUYS )
	 	local vRandomOffset = Vector(RandomInt(-300, 300), RandomInt(-300, 300), 0)
	 	local vSpawnPoint =vPosition + vRandomOffset
		 FindClearSpaceForUnit(hHoundBoss1, vSpawnPoint, true)
		 FindClearSpaceForUnit(hHoundBoss2, vSpawnPoint, true)
