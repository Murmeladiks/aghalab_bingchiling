...
function CBossPrimalBeast:SetEncounter( hEncounter )
	...
	-- local vPosition = self.HomePosition
	-- 	local hGuardianBoss1 = CreateUnitByName( "npc_dota_creature_kobold", self.HomePosition, true, nil, nil, DOTA_TEAM_BADGUYS )
	--  	local vRandomOffset = Vector(RandomInt(-300, 300), RandomInt(-300, 300), 0)
	--  	local vSpawnPoint =self.HomePosition + vRandomOffset
...
function CBossPrimalBeast:EvaluateOnslaughtAbility()
	...
	if #hEnemies > 0 then
		...
		local position = self.me:GetAbsOrigin() + self.me:GetForwardVector()*2000
